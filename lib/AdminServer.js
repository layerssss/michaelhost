    ...
    adminServer.on("upgrade", (request, socket, head) =>
      Promise.resolve()
        .then(async () => {
          ...
          const composeApplicationPathMatch = url.match(/\/api\/compose_applications\/([\w]{8})/);
          if (composeApplicationPathMatch) {
            const composeApplicationId = composeApplicationPathMatch[1];
            const composeApplication = state.composeApplications.find(t => t.id === composeApplicationId);
            if (!composeApplication)
              throw new Error(`Cannot find ComposeApplication#${composeApplication}`);
