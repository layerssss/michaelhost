...
class State {
  ...
  handleWebSocketConnect({ session }) {
    session.errorListener = ({ error, errorInfo }) => {
    ...
    createLogger.globalEventEmitter.on("error", session.errorListener);
  }

  handleWebSocketClose({ session }) {
    createLogger.globalEventEmitter.off("error", session.errorListener);
  }
