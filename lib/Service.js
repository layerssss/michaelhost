...
class Service {
  static async init({
    ...
    const webhookServer = await WebhookServer.init({ state: state });
  ...
  async start() {
    if (this.webhookServerPort)
      await serverListenAsync(this.webhookServer, {
  ...
  async stop() {
    ...
    if (this.webhookServerPort) await serverStopAsync(this.webhookServer);
