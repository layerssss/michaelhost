...
class ReverseProxy {
  ...
  async proxyRequest({ request, response, target }) {
    await new Promise((resolve, reject) => {
      response.on("close", resolve);
      this.proxy.web(request, response, { target }, error => reject(error));
    });
  }

  async proxyUpgrade({ request, socket, head, target }) {
    await new Promise((resolve, reject) => {
      socket.on("close", resolve);
      this.proxy.ws(request, socket, head, { target }, error => reject(error));
    });
  }
  ...
  handleRequest({ request, response, ssl }) {
    Promise.resolve()
      .then(async () => {
        ...
        const host = this.state.hosts.find(h => h.hostname === hostname);
        if (!host) throw new Error(`${hostname} not found.`);
        ...
        await this.proxyRequest({ request, response, target: host.upstream });
      })
      .catch(error => {
        logger.error(error);
  ...
  redirect({ response, redirectUrl }) {
    ...
    response.end(`<a href="${redirectUrl}">${redirectUrl}</a>`);
  }
  ...
  handleUpgrade({ request, socket, head, ssl }) {
    Promise.resolve()
      .then(async () => {
        ...
        if (host.ssl !== ssl) throw new Error(`Please use: ${host.origin}/`);
