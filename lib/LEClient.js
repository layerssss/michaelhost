...
class LEClient {
  ...
  async handleSNICallback({ domain }) {
    const host = this.state.hosts.find(h => h.hostname === domain);
    if (!host) throw new Error(`Cannot find host: ${host}`);
  ...
  async getDomainCert({ domain }) {
    ...
    if (domainCert) {
      ...
      if (Number(cert.validity.notAfter) > Date.now() + 3600 * 1000)
        return domainCert;
    }
