...
const INFO_STATE_HISTORY_SIZE = 100000;
const BLACKBOX_SIZE = 1000;

const prettyStdOut = new PrettyStream();
prettyStdOut.pipe(process.stdout);

const blackboxStream = new PrettyStream({ mode: "short", useColor: false });
blackboxStream.on("data", (data) => {
  createLogger.blackbox += data;
  createLogger.blackbox = createLogger.blackbox.slice(-BLACKBOX_SIZE);
});
...
prettyInfo.on("data", (dataString) => {
  ...
  infoState.history = infoState.history.slice(-INFO_STATE_HISTORY_SIZE);
...
Object.assign(createLogger, { infoState, globalEventEmitter, blackbox: "" });
